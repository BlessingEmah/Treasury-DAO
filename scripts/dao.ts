...
async function main() {
  ...
  const deployDao = await DAO.deploy();
  ...
  const value = { value: ethers.utils.parseEther("10") };
  //const value1 = ethers.utils.parseEther("0.1");
  await deployDao.contribute(value);

  const [investor1, investor2, investor3, address4, investor4, investor5] =
  ...
  console.log(await deployDao.getShares());

  //uncomment the line below to check the shares of an investor.
  console.log("The Investor2 shares is:",await deployDao.connect(investor2).shares(investor2.address))

  // const value2 = ethers.utils.parseEther("0.1");
  await deployDao.connect(investor2).transferShare(1, investor3.address)
  console.log(
  "investor2 transferred:",
  await deployDao.connect(investor2).shares(investor2.address)
  );

  console.log(await deployDao.getS(investor2.address));
    console.log(await deployDao.getS(investor3.address));
